...
const periodicTableElements = require("./elements.js");
const { sBlock, pBlock, dBlock, fBlock } = require("./blocks.js");
...
class Server {
  ...
  create(socket, data) {
    if (
      !data.nickname ||
      (this.players.filter((e) => e.id === socket.id)[0] &&
        this.rooms.get(
          this.players.filter((e) => e.id === socket.id)[0].roomID
        )) ||
      this.players.filter((player) => player.id === socket.id).length > 1
    )
      return; // owner nick and id
    const room = {};
    do {
    ...
    room.self_destruction = () => {
      room.players.forEach((player) => {
        ...
        this.players.splice(
          this.players.findIndex((player_) => player_.id === player.id),
          1
        );
      });
    ...
    room.timeout = setTimeout(
      room.self_destruction,
      1e3 * this.config.room_timeout
    );
    socket.send(JSON.stringify({ type: "created_room", id: room.roomID }));
    this.rooms.set(room.roomID, room);
    log.log(
      `Room \x1b[35m${room.roomID}\x1b[0m created by \x1b[32m${data.nickname}\x1b[0m`
    );
  ...
  join(socket, data) {
    ...
    if (!data.nickname.replace(/\s+/g, "").length)
      return socket.send(
        JSON.stringify({
          type: "error",
          title: "Couldn't Join",
          message: "You don't have a valid nickname!",
        })
      );
    ...
    if (
      room.players.filter((player) => player.id === socket.id).length ||
      this.players.filter((player) => player.id === socket.id).length > 1
    )
      return socket.send(
        JSON.stringify({
    ...
    const player = {
      ...
      points: 0,
      chatColor:
        room.chatColors.filter(
          (color) =>
            !room.players.filter((player) => player.chatColor === color).length
        )[
          ~~(
            Math.random() *
            room.chatColors.filter(
              (color) =>
                !room.players.filter((player) => player.chatColor === color)
                  .length
            ).length
          )
        ] || "#fff",
    ...
    if (this.config.nick_badword_filter && badwords.check(player.nickname)) {
      log.log(
        `Player \x1b[32m${player.nickname}\x1b[0m was kicked from \x1b[35m${room.roomID}\x1b[0m due to having improper nickname`
      );
      socket.send(
        JSON.stringify({
          type: "error",
          title: "Couldn't Join",
          message: "Improper nickname!",
        })
      );
      this.leave(socket);
      return;
    ...
    log.log(
      `Player joined \x1b[32m${player.nickname}\x1b[0m to \x1b[35m${room.roomID}\x1b[0m`
    );
  }
  leave(socket) {
    if (
      !this.players.filter((player) => player.id === socket.id).length ||
      !this.rooms.get(
        this.players.filter((player) => player.id === socket.id)[0].roomID
      )
    )
      return;
    ...
    log.log(
      `Player left \x1b[32m${player.nickname}\x1b[0m from \x1b[35m${room.roomID}\x1b[0m`
    );
    if (room.isRunning)
      this.sendMessage(room, null, player.nickname + " left the game!");
    this.players.splice(
      this.players.findIndex((player) => player.id === socket.id),
      1
    );
    ...
    if (room.ownerID === socket.id) {
      ...
      room &&
      ((room.isRunning && room.players.length <= 1) ||
  ...
  kick(socket, data) {
    ...
    if (!kickedPlayer.length || !kickedPlayer[0].socket)
      return socket.send(
        JSON.stringify({
          type: "error",
          title: "Couldn't kick a player",
          message: "Player not found!",
        })
      );
    if (kickedPlayer[0].id === socket.id)
      return socket.send(
        JSON.stringify({
          type: "error",
          title: "Couldn't kick a player",
    ...
    log.log(
      `Player \x1b[32m${player.nickname}\x1b[0m has been kicked from \x1b[35m${room.roomID}\x1b[0m by owner`
    );
    kickedPlayer[0].socket.send(
      JSON.stringify({
        type: "error",
        title: "Kicked",
        message: "You have been kicked by owner!",
      })
    );
  ...
  start(socket) {
    ...
    if (room.players.length < 2)
      return socket.send(
        JSON.stringify({
          type: "error",
          title: "Couldn't start the game",
          message: "Not enough players! (Minimum 2)",
        })
      );
    if (room.isRunning)
      return socket.send(
    ...
    room.players.forEach((player) => {
      player.socket.send(
        JSON.stringify({ type: "start", playerCount: room.players.length })
      );
    });
    this.updateRoom(room.roomID);
  ...
  action(socket, data) {
    ...
    if (data.card && data.card.number)
      data.card.number = isNaN(parseInt(data.card.number))
        ? null
      ...
      case "place": {
        ...
        if (player.turn === "gmatch") {
          ...
          if (
            ...
            data.card.group !== lastNumberedCard.group
          ) {
            return;
          ...
          if (
            ...
            data.card.period !== lastNumberedCard.period
          ) {
            return;
          } else if (
            data.card &&
          ...
          if (groupMatchCards.length > 0) {
            this.nextTurn(room, player, data, socket, "pmatch");
          } else {
            room.centerPlus = room.centerPlus + 3;
            this.nextTurn(room, player, data, socket, "pmatch");
          }
          ...
          if (
            ...
            data.card.pickedColor === "blue" ||
            data.card.pickedColor === "yellow"
          ) {
            room.centerCard = data.card;
        ...
        } else if (player.turn === "plus4" && data.card.ability !== "plus2")
          return;
        if (
          (data.card.color &&
            player.turn !== "plus4" &&
          ...
          (data.card.ability === "plus2" &&
            data.card.color === room.centerColor &&
            player.turn === "plus4")
        ) {
          if (
            room.centerCard.color === data.card.color ||
            (room.centerColor === data.card.color && data.card.color) ||
            (room.centerCard.number === data.card.number &&
              data.card.number !== null) ||
            (room.centerCard.ability === data.card.ability && data.card.ability)
          ) {
            room.centerCard = data.card;
      ...
      if (!room.cards.length) {
        if (!room.centerCards.length) {
          this.sendMessage(
            room,
            null,
            "No cards left in draw pile and discard pile! Restarting the game."
          );
  ...
  nextTurn(room, player, data, socket, turn = true) {
    ...
    if (room.reversed) {
      ...
      if (
        (turn === "block" ||
          (turn === "reverse" && room.players.length === 2)) &&
        0 > index
      )
  ...
  turn(player, turn = true) {
    ...
    player.socket.send(
      JSON.stringify({ type: "timeout", time: this.config.player_timeout })
    );
    player.timeout = setTimeout(() => {
      player.socket.send(
        JSON.stringify({
          type: "error",
          title: "Timed Out",
          message: "You were kicked due to inactivity!",
        })
      );
  ...
  next(code) {
    ...
    room.players.forEach((player) => {
      player.cards.forEach((card) => {
        ...
        if (card && (card.ability === "plus4" || card.ability === "change"))
          winPoints += 50;
    ...
    if (this.config.announce_round_winner && winner)
      this.sendMessage(
        room,
        null,
        `${winner.nickname} won this round! (+${winPoints} points)`
      );
    const scoresList = [];
    room.players.forEach((player) => {
      scoresList.push({
    ...
    if (winner.points >= this.config.win_points)
      return setTimeout(() => {
        room.players.forEach((player) => {
          player.socket.send(
            JSON.stringify({ type: "winner", winner: winner.nickname })
          );
          player.points = 0;
        });
        this.sendMessage(
          room,
          null,
          "Congratolations to " + winner.nickname + " for winning the game!"
        );
  ...
  uno(socket) {
    ...
    if (
      ...
      !room.isRunning ||
      (socket.lastUno &&
        socket.lastUno > Date.now() - this.config.uno_cooldown &&
        !(
          room.players.filter((player) => player.turn).length &&
          room.players.filter((player) => player.turn)[0].id === socket.id &&
          room.players.filter((player) => player.turn)[0].cards.length <= 2 &&
          !room.players.filter((player) => player.turn)[0].uno
        ))
    )
      return;
    ...
    room.players.forEach((player) => {
      ...
      if (
        player.id === socket.id &&
        ((player.cards.length <= 2 && player.cards.length && player.turn) ||
          (player.cards.length === 1 && !player.uno))
      )
        player.uno = true;
      if (player.cards.length === 1 && !player.uno) {
        ...
        room.players.forEach((player_) => {
          player_.socket.send(
            JSON.stringify({ type: "uno", nickname: player.nickname })
          );
        });
        player.uno = true;
        this.updateRoom(room.roomID);
  ...
  message(socket, data) {
    if (
      (socket.lastMessage &&
        socket.lastMessage > Date.now() - this.config.chat_cooldown) ||
      !data.message ||
      !data.message.replace(/\s+/g, "").length
    )
      return;
    const player = this.data(socket).player;
    const room = this.data(socket).room;
    ...
    if (data.message.split("")[0] === "/")
      return this.playerCommands.exec(socket, data.message.slice(1));
    this.sendMessage(
      room,
      player.nickname,
      this.config.chat_badword_filter
        ? badwords.clear(data.message)
        : data.message,
      player.chatColor
    );
    if (this.config.log_chat)
      log.log(
        "\x1b[35m" + room.roomID + "\x1b[0m",
        "-",
        player.nickname + ":",
        "\x1b[90m" + data.message + "\x1b[0m"
      );
  }
  ...
  roomList(socket) {
    if (
      socket.lastRefresh &&
      socket.lastRefresh > Date.now() - this.config.refresh_cooldown
    )
      return;
    const rooms = new Array();
    this.rooms.forEach((room) => {
      if (!room.private && !room.isRunning)
        rooms.push({
          roomID: room.roomID,
          playerCount: room.players.length,
          roomName:
            (room.players.filter((player) => player.id === room.ownerID)[0]
              ? room.players.filter((player) => player.id === room.ownerID)[0]
                  .nickname
              : "Unnamed") + "'s room",
    ...
    socket.send(
      JSON.stringify({
        type: "room_list",
        rooms: rooms.sort((a, b) => a.playerCount - b.playerCount),
        cooldown: this.config.refresh_cooldown,
      })
    );
  ...
  main() {
    ...
    this.server.on("connection", (socket) => {
      if (
        ...
        this.server.clients.size > this.config.max_connections
      )
          ...
          switch (data.type) {
            ...
            case "room_check": {
              if (
                ...
                !this.rooms.get(data.roomID).isRunning &&
                this.rooms.get(data.roomID).players.length < 4
              )
                socket.send(
                  JSON.stringify({ type: "room_check", callback: true })
                );
              else if (!this.rooms.get(data.roomID))
                socket.send(
                  JSON.stringify({
                    type: "room_check",
                    callback: false,
              ...
              else if (this.rooms.get(data.roomID).isRunning)
                socket.send(
                  JSON.stringify({
                    type: "room_check",
                    callback: false,
                    message: "Room is running!",
                  })
                );
              else if (this.rooms.get(data.roomID).players.length >= 4)
                socket.send(
                  JSON.stringify({
                    type: "room_check",
                    callback: false,
  ...
  updateCards(room) {
    room.players.forEach((player) => {
      player.socket.send(
        JSON.stringify({ type: "my_cards", cards: player.cards })
      );
    });
  }
  updateRoom(code) {
    ...
    if (!room) return;
    room.players.forEach((player) => {
      let fakeroom = JSON.parse(
        JSON.stringify(room, this.getCircularReplacer())
      );
      if (room.isRunning) {
  ...
  updatePlayers(room) {
    if (
      ...
      this.turn(room.players[~~(room.players.length * Math.random())]);
    room.players.forEach((player) => {
      player.socket.send(
        JSON.stringify({
          type: "players_update",
