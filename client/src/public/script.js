...
server.onclose = (close) => {
  ...
  $(".error-head .error-desc").text(
    (close && close.reason) || "Please reload the page!"
  );
  $(".error-close").hide();
...
server.onmessage = function (event) {
  ...
  if (data.type === "room_list") {
    ...
    if (!data.rooms.length)
      return $(".room-list").html("No rooms avainable :(");
    data.rooms.forEach((room) => {
      $(".room-list").append(
        `<div class="item" data-id="${room.roomID}"><div><a title="${escapeHtml(
          room.roomName
        )}">${escapeHtml(room.roomName)}</a></div><div><a>Players ${
          room.playerCount
        }/4</a></div></div>`
      );
    });
  ...
  if (data.type === "my_cards") {
    ...
    data.cards.forEach((card) => {
      ...
      if (card.ability === "reverse")
        return $(".you .my-cards").append(
          `<div class="my-card num-reverse ${card.color}" data-ability="reverse" data-color="${card.color}" data-number><span class="inner"><span class="mark">↩</span></span></div>`
        );
      ...
      if (card.ability === "change")
        return $(".you .my-cards").append(
          `<div class="my-card num-change black" data-ability="change" data-color data-number><span class="inner"><span class="mark"><div class="change"><div class="segment yellow"></div><div class="segment green"></div><div class="segment blue"></div><div class="segment red"></div></div></span></span></div>`
        );
    });
    $(".my-cards .my-card").click((card) => {
      ...
      if (
        ...
        card.currentTarget.getAttribute("data-ability") === "plus4"
      )
        $(".color-selector-container").show();
      else
        server.send(
          JSON.stringify({ type: "action", card: user.push, action: "place" })
        );
    });
  ...
  if (data.type === "room") {
    if (!data.room.isRunning) {
      if (
        room.players.length !== data.room.players.length ||
        user.roomID !== data.room.roomID
      ) {
        room.players = data.room.players;
        user.roomID = room.roomID;
        $(".limbo-player-list").html("");
        room.players.forEach((player) => {
          $(".limbo-player-list").append(
            `<div class="limbo-player-item"${
              user.action === "create" ? ` data-id="${player.id}"` : ""
            }>${escapeHtml(player.nickname)}</div>`
          );
            ...
            $(".limbo-player-list .limbo-player-item").click((element) => {
              ...
              $(".kick-accept").click(() => {
                server.send(
                  JSON.stringify({
                    type: "kick",
                    id: element.currentTarget.getAttribute("data-id"),
                  })
                );
              ...
              $("#player-management-container .kick-head .kick-desc a").text(
                element.currentTarget.textContent
              );
              $("#player-management-container").show();
            });
        });
        if (user.action === "create")
          $(".limbo-player-item").css({
            "pointer-events": "all",
            cursor: "pointer",
          });
        else
      ...
      if (!isNaN(data.room.centerCard.number) && !data.room.centerCard.ability)
        $(".center .center-card").html(
          `<div class="my-card num-${data.room.centerCard.number} ${data.room.centerCard.color}" data-ability data-color="${data.room.centerCard.color}" data-number="${data.room.centerCard.number}"><span class="inner"><span class="mark">${data.room.centerCard.symbol}</span></span></div>`
        );
      ...
      if (data.room.centerCard.ability === "plus2")
        $(".center .center-card").html(
          `<div class="my-card num-plus2 ${data.room.centerCard.color}" data-ability="plus2" data-color="${data.room.centerCard.color}" data-number><span class="inner"><span class="mark">+2</span></span></div>`
        );
      if (data.room.centerCard.ability === "reverse")
        $(".center .center-card").html(
          `<div class="my-card num-reverse ${data.room.centerCard.color}" data-ability="reverse" data-color="${data.room.centerCard.color}" data-number><span class="inner"><span class="mark">↩</span></span></div>`
        );
      ...
      if (data.room.centerCard.ability === "change")
        $(".center .center-card").html(
          `<div class="my-card num-change ${data.room.centerColor}" data-ability="change" data-color data-number><span class="inner"><span class="mark"><div class="change"><div class="segment yellow"></div><div class="segment green"></div><div class="segment blue"></div><div class="segment red"></div></div></span></span></div>`
        );
      let you = data.room.players.findIndex((player) => player.id === user.id);
      $(`.player.you .player-nickname`).html(
        `<a${data.room.players[you].turn ? ' class="turn"' : ""}>${escapeHtml(
          data.room.players[you].nickname
        )}</a>`
      );
      for (let j = 0, i = you + 1; true; i++) {
        ...
        data.room.players[i].cards.forEach(() => {
          $(`.player.num-${data.room.players.length}-${j} .my-cards`).append(
            `<div class="my-card black"><span class="inner"><span class="mark default"><div class="uno-default">CHEMO</div></span></span></div>`
          );
        });
        $(`.player.num-${data.room.players.length}-${j} .player-nickname`).html(
          `<a${data.room.players[i].turn ? ' class="turn"' : ""}>${escapeHtml(
            data.room.players[i].nickname
          )}</a>`
        );
      }
  ...
  if (data.type === "start") {
    ...
    for (let j = 1; j < data.playerCount; j++) {
      $("#overlay")
        .append(`<div class="player num-${data.playerCount}-${j} opponent">
  ...
  if (data.type === "uno") {
    ...
    $(".uno-message").hide();
    $(".uno-message").text(
      data.nickname + " didn't press the UNO button! (+2 cards)"
    );
    $(".uno-message").show();
    window.uno_message = setTimeout(() => {
      $(".uno-message").hide();
    }, 4e3);
  }
  if (data.type === "win") {
    if (!isNaN(data.centerCard.number) && !data.centerCard.ability)
      $(".center .center-card").html(
        `<div class="my-card num-${data.centerCard.number} ${data.centerCard.color}" data-ability data-color="${data.centerCard.color}" data-number="${data.centerCard.number}"><span class="inner"><span class="mark">${data.centerCard.number}</span></span></div>`
      );
    if (data.centerCard.ability === "plus4")
      $(".center .center-card").html(
    ...
    if (data.centerCard.ability === "reverse")
      $(".center .center-card").html(
        `<div class="my-card num-reverse ${data.centerCard.color}" data-ability="reverse" data-color="${data.centerCard.color}" data-number><span class="inner"><span class="mark">↩</span></span></div>`
      );
    ...
    if (data.centerCard.ability === "change")
      $(".center .center-card").html(
        `<div class="my-card num-change ${data.centerColor}" data-ability="change" data-color data-number><span class="inner"><span class="mark"><div class="change"><div class="segment yellow"></div><div class="segment green"></div><div class="segment blue"></div><div class="segment red"></div></div></span></span></div>`
      );
    ...
    JSON.parse(JSON.stringify(data.players))
      ...
      .forEach((player) => {
        $(".scores-list").append(
          `<div class="score"><div>${escapeHtml(player.nickname)}</div><div${
            player.isWinner ? ' style="color:#00ff00;"' : ""
          }>${player.isWinner ? "+" : ""}${player.points}/${
            data.max_points
          }</div></div>`
        );
      });
      ...
      data.players[i].cards.forEach((card) => {
        if (!isNaN(card.number) && !card.ability)
          return $(`.player.num-${data.players.length}-${j} .my-cards`).append(
            `<div class="my-card num-${card.number} ${card.color}" data-ability data-color="${card.color}" data-number="${card.number}"><span class="inner"><span class="mark">${card.number}</span></span></div>`
          );
        if (card.ability === "plus4")
          return $(`.player.num-${data.players.length}-${j} .my-cards`).append(
            `<div class="my-card num-plus4 black" data-ability="plus4" data-color data-number><span class="inner"><span class="mark">+4</span></span></div>`
  ...
  if (data.type === "chat_message") {
    $(".chat-messages").append(`<div class="chat-item">
                        <div class="sender" style="color:${
                          data.color
                        }">${escapeHtml(data.nickname)}:</div>
  ...
  if (user.action === "create") {
    server.send(
      JSON.stringify({
        ...
        private: $("[data-private]").is(":checked"),
      })
    );
  }
  if (user.action === "join") {
...
$(".chat-send").click(() => {
  server.send(
    JSON.stringify({ type: "message", message: $(".chat-input input").val() })
  );
...
if (localStorage.getItem("nick"))
  $("input.nickname").val(localStorage.getItem("nick"));
if (localStorage.getItem("chat_open") !== "true") {
  $(".chat").css("height", "0px");
  $(".chat-switch i").css("transform", "rotate(180deg)");
}

var escapeHtml = (message) =>
  message.replace(/[\"&<>]/g, (off) => {
    return {
      '"': "&quot;",
      "'": "&#39;",
      "&": "&amp;",
